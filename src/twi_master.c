...
{
    // Transmit 1 byte
    LOG("Write data byte: 0x%02X...", data);
    TWDR = data;
    TWCR = (1 << TWINT) | (1 << TWEN);
...
static uint8_t tw_read(bool read_ack)
{
    ...
    uint8_t data = TWDR;
    LOG("Read data byte: 0x%02X\n", data);
    return data;
}

#define TWBR_CALC(FREQ) 0.5 * ((F_CPU/FREQ)-16)

void tw_init(twi_freq_mode_t twi_freq_mode, bool pullup_en)
{
    TW_DIR |= (1 << TW_SDA_PIN) | (1 << TW_SCL_PIN);
    if (pullup_en)
    {
        LOG("Enable pull-up resistor.");
